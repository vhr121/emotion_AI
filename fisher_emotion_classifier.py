...
expression = ['happy', 'sad', 'surprise', 'angry','hunger','assistance']
exp_dict = {'happy': 0, 'sad': 1, 'surprise': 2, 'angry': 3,'hunger':4,'assistance':5}
exp_index = [0, 1, 2, 3, 4, 5]
samples = 20
old_expr = 'wait!'
...
if args.update:
    ...
    fisher_update_model.update(expression)
        ...
        actions["hunger"] = [x for x in df.hunger.dropna()]
        actions["assistance"] = [x for x in df.assistance.dropna()]


        fisher = cv2.createFisherFaceRecognizer()
        try:
            fisher.load("models/fisher/trained_fisher_emoclassifier_2.xml")
        except:
