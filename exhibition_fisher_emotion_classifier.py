...
import cv2
import fisher_update_model
import pandas
import statistics

laptop_camera = 0
web_camera = 1
Capture = cv2.VideoCapture(web_camera)
expression = ['happy', 'sad', 'surprise', 'angry', 'assistance']
exp_dict = {'happy': 0, 'sad': 1, 'surprise': 2, 'angry': 3,  'assistance': 4}
exp_index = [0, 1, 2, 3, 4]
samples = 20
old_expr = 'wait....!'
filename = 'trained_fisher_emoclassifier_11.xml'

x_cord, y_cord, width, height = 0, 30, 0, 0

# take the options from the users in form of arguments
...
def read_img_roi():
    ...
    face_cord = face_cascade.detectMultiScale(clahe_image, scaleFactor=1.1, minNeighbors=15, minSize=(10, 10),
    ...
    if len(face_cord) == 1:
        ...
        return face_img, face_cord, True
...
def make_training_data(exp):
    global d_x_cord, d_y_cord, d_height, d_width
    d_x_cord, d_y_cord, d_height, d_width = 0, 30, 0, 0
    ...
    while remaining_to_read > 0:
        try:
            ret, frame = Capture.read()
            cv2.rectangle(frame, (d_x_cord, d_y_cord), (d_x_cord + d_height, d_y_cord + d_width), (255, 0, 0), 3)
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(frame, 'training', (d_x_cord, d_y_cord + d_height + 30), font, 1.5, (255, 255, 255), 2,
                        cv2.CV_AA)
            cv2.imshow('frame', frame)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                exit(0)
            face_image, face_cord, ret = read_img_roi()
            for x, y, w, h in face_cord:
                d_x_cord = x
                d_y_cord = y
                d_height = h
...
if args.update:
    ...
    fisher_update_model.update(expression)
    ...
    actions["assistance"] = [x for x in df.assistance.dropna()]

    fisher = cv2.createFisherFaceRecognizer()
    try:
        fisher.load("models/fisher/%s" % filename)
    except:
    ...
    while 1:
        ...
        while count > 0:
            ...
            while local_count > 0:
                ...
                ret, frame = Capture.read()
                cv2.rectangle(frame, (x_cord, y_cord), (x_cord + width, y_cord + height), (255, 0, 0), 3)
                font = cv2.FONT_HERSHEY_SIMPLEX
                if args.update:
                    cv2.putText(frame, 'training', (x_cord, y_cord + height + 30), font, 1.5, (255, 255, 255), 2,
                                cv2.CV_AA)
                else:
                    cv2.putText(frame, old_expr, (x_cord, y_cord + height + 30), font, 1.5, (255, 255, 255), 2,
                                cv2.CV_AA)
                cv2.imshow('frame', frame)
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    exit(0)

            face, face_cord, ret = read_img_roi()
            ...
            if ret == True:
                for x, y, w, h in face_cord:
                    ...
                    width = w
                    height = h
                pred, confidence = fisher.predict(face)
